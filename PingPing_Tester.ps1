...
$scopelist = Get-Content .\ping.txt
$global:group = $()
$times = 4
$packet_size = 1452
...
#Export a log file
$logs=$group | export-csv .\logs.csv -Append -NoTypeInformation -Encoding Default
...
$Alert_destname | foreach -process{
    if ($_ -ne $null){
    [System.Windows.Forms.MessageBox]::Show("$_ is Down!"," Connection Alert")
    }

    else{[System.Windows.Forms.MessageBox]::Show("The requests are all suceeded!","Connection Alert")
    }
